# 0x16. C - Simple Shell
This directory contains "Everything you need to know to start coding your own shell"
tasks for the simple shell project.
...
In order to retrieve the current process’ ID, you can use the system call `getpid` (man 2 `getpid`):
...
It is possible to get the PID of a parent process by using the
getppid system call (man 2 `getppid`), from within the child process.
...
important make sure you read the man, and the RETURN VALUE section, in order to know
when to stop reading Keyword: “end-of-file”, or EOF (or Ctrl+D).
...
_#advanced: Write the function without strtok_

## Executing a program
The system call `execve` allows a process to execute another program (man 2 `execve`).
Note that this system call does load the new program into the current process’ memory in place
of the “previous” program: on success execve does not return to continue the rest of the “previous” program.

**Warning:** in this example, `execve` is used without the current environment (last argument),
...
Once fork successfully returns, two processes continue to run the same program,
...
is the father or the child: `fork` will return `0` to the child, and the PID of the child to the father.
...
## Environment
We have seen earlier that the shell uses an environment list, where environment variables are “stored”.
The list is an array of strings, with the following format: `var=value`,
where var is the name of the variable and value its value. As a reminder,
you can list the environment with the command printenv:
...
Actually, every process comes with an environment.
When a new process is created, it inherits a copy of its parent’s environment.
To access the entire environment within a process, you have several options:
...
#### 1. env vs environ
Write a program that prints the address of `env` (the third parameter of the main function)
and `environ` (the global variable). Are they they same? Does this make sense?
